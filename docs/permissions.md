...
- `get_attempt_review(p_attempt_id)` — SECURITY DEFINER review payload. Permissioned admin (`view_students` / `view_analytics` via `can_admin_select_attempt_detail`) **or** owner with `account_allows_lms()`; then `show_review`; **and** for paid tests the student must have **live** content entitlement (`resource_content_allowed`) before stems, selected keys, correct keys, or explanations are returned. Blocked student accounts (`restricted` / `suspended` / `deactivated` / `revoked`) raise `account_not_eligible` even for their own submitted attempt — review is not part of Layer 2 live-exam continuation. Denied paid review is `{ allowed: false, reason: 'review_locked_entitlement' }` with **no items**. Score/result summary is not this RPC (`get_own_test_result`). After expiry, renewal may restore review; it never rescores.

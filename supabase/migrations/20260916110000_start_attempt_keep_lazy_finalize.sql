-- Fix: start_attempt must not RAISE after finalize_if_expired.
-- RAISE aborts the transaction and rolls the auto-submit back, so
-- close-now → expiry → resume redirected to /result with get_own_test_result NULL.
-- Existing submitted attempts still raise already_submitted (no work to lose).
-- Resume that lazy-finalizes returns already_submitted=true on a committed row.
...
begin
  ...
  -- Committed lazy-finalize: returning a flag keeps the submit. Raising
  -- already_submitted here used to roll back finalize_if_expired.
...
comment on function public.start_attempt(uuid, uuid) is
  'Start or resume. Lazy-finalize of an expired in_progress returns already_submitted=true without rolling back the submit. A prior submitted row still raises already_submitted.';
